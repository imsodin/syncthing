...
func (t *nonrecursiveTree) Watch(path string, c chan<- EventInfo,
	...
	fmt.Printf("[NOTIFYDEBUG] nonrecursiveTree.Watch: %v\n", path)
	path, isrec, err := cleanpath(path)
	if err != nil {
		fmt.Printf("[NOTIFYDEBUG] nonrecursiveTree.Watch: failed cleaning path %v: %v\n", path, err)
		return err
	}
	...
	defer t.rw.Unlock()
	nd := t.root.Add(path)
	fmt.Printf("[NOTIFYDEBUG] nonrecursiveTree.Watch (path=%v): isrec: %v\n", path, isrec)
	if isrec {
