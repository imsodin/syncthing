...
func Open(path string, tuning Tuning) (Backend, error) {
	if os.Getenv("USE_BOLT") != "" {
		l.Warnln("Using experimental bolt db")
		if err := maybeCopyDatabase(path, strings.Replace(path, locations.BoltDir, locations.LevelDBDir, 1), OpenBolt, OpenLevelDBRO); err != nil {
			return nil, err
		}
		return OpenBolt(path)
	}

	if err := maybeCopyDatabase(path, strings.Replace(path, locations.LevelDBDir, locations.BoltDir, 1), OpenLevelDBAuto, OpenBolt); err != nil {
		return nil, err
	}
