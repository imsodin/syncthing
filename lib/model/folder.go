...
func (b *scanBatch) Update(fi protocol.FileInfo, snap *db.Snapshot) bool {
	...
	// Resolve receive-only items which are identical with the global state.
	switch gf, ok := snap.GetGlobal(fi.Name); {
	case !ok:
	case gf.IsReceiveOnlyChanged():
		// The global item is ours
		if fi.IsDeleted() {
			l.Debugf("%v scanning: Marking item as not locally changed", b.f, fi)
			if b.f.Type == config.FolderTypeReceiveOnly {
				fi.LocalFlags &^= protocol.FlagLocalReceiveOnly
			} else {
				fi.LocalFlags = protocol.FlagLocalReceiveEncryptedDeleted
			}
		}
			...
			case file.IsDeleted() && file.IsReceiveOnlyChanged():
				...
				case config.FolderTypeReceiveOnly, config.FolderTypeReceiveEncrypted:
					if gf, _ := snap.GetGlobal(file.Name); gf.IsDeleted() {
						...
						return batch.Update(file.ConvertDeletedToFileInfo(), snap)
					}
				default:
