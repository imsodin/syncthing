...
func TestRecvOnlyRevertDeletes(t *testing.T) {
	...
	for _, dir := range []string{".stfolder", "ignDir", "unknownDir"} {
		must(t, func() error { return ffs.MkdirAll(dir, 0755) })
	}
	must(t, func() error {
		return ioutil.WriteFile(filepath.Join(ffs.URI(), "ignDir", "ignFile"), []byte("hello\n"), 0644)
	})
	must(t, func() error {
		return ioutil.WriteFile(filepath.Join(ffs.URI(), "unknownDir", "unknownFile"), []byte("hello\n"), 0644)
	})
	must(t, func() error { return ioutil.WriteFile(filepath.Join(ffs.URI(), ".stignore"), []byte("ignDir\n"), 0644) })
...
func TestRecvOnlyRevertNeeds(t *testing.T) {
	...
	must(t, func() error { return ffs.MkdirAll(".stfolder", 0755) })
	oldData := []byte("hello\n")
	...
	must(t, func() error {
		return ioutil.WriteFile(filepath.Join(ffs.URI(), "knownDir", "knownFile"), newData, 0644)
	})
...
func TestRecvOnlyUndoChanges(t *testing.T) {
	...
	must(t, func() error {
		return ioutil.WriteFile(filepath.Join(ffs.URI(), "knownDir", "knownFile"), []byte("bye\n"), 0644)
	})
...
func setupKnownFiles(t *testing.T, ffs fs.Filesystem, data []byte) []protocol.FileInfo {
	...
	must(t, func() error { return ffs.MkdirAll("knownDir", 0755) })
	must(t, func() error { return ioutil.WriteFile(filepath.Join(ffs.URI(), "knownDir", "knownFile"), data, 0644) })

	t0 := time.Now().Add(-1 * time.Minute)
	must(t, func() error { return ffs.Chtimes("knownDir/knownFile", t0, t0) })
